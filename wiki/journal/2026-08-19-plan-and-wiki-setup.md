...
- GitHub 원격 연결: `git remote add origin https://github.com/kimdoogi/java-heavy-traffic.git` → 원격 비어 있음 확인 → 레포 로컬 identity를 개인 메일로 설정(전역은 회사 메일) → 첫 커밋 `95dc114` → `git push -u origin main` 성공. 커밋 정책 → [howto/git.md](../howto/git.md).
...
- [x] git 커밋 정책 확정 (작업 단위마다 커밋, 세션 종료 시 push)
