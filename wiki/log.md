...
- 2026-08-27 | redis 제품화 | E6 결론(스케일=redis) 반영 — 잔여수량 count 기반 정확화(GET, redis remaining stale 대응), 크래시 갭 @Scheduled 조정(redis 발급자 set↔DB 대조 전진복구, 멱등), README 데이터 기반 포트폴리오 얼굴로 갱신. 코드 uncommitted(리뷰 대기) | [redis-atomic-stock](concepts/redis-atomic-stock.md)
