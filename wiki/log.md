...
- 2026-08-27 | E6 실험 도구 작성 (Step1) | k6 flash-sale 시나리오 + verify/reset 스크립트. 스모크 통과(재고10→발급10·sold_out40·초과0), setResponseCallback·완주 threshold 검증. ⚠️scripts/=A소유·index는 A가 E6툴 예정으로 기록 → 트랙 충돌, 협의 필요 | [journal](journal/2026-08-27-e6-flash-sale.md)
- 2026-08-27 | E6 실행 완료 (4전략) | pool=50=VUS 격리(풀 비병목, unexpected=0). none 초과 +4,000(무락 붕괴)·pessimistic 520rps·optimistic 재시도폭증 503 72%·redis 549rps 최고, 락 3전략 초과0. 초기 pool=20/1,000VU는 커넥션 타임아웃 1,126건으로 비교 불가 → 풀 격리 필요 실측 | [E6](experiments/E6-flash-sale-consistency.md) · [redis-atomic-stock](concepts/redis-atomic-stock.md)
- 2026-08-27 | E6 크로스오버 (VUS 50→1000) | pool=80 고정, pessimistic vs redis 둘 다 무붕괴·처리량 동률(~1,400-1,600rps@1000VU), 초과0·500=0. 1cpu선 병목이 앱 CPU라 안 갈림 | [E6 크로스오버](experiments/E6-crossover-concurrency.md)
- 2026-08-27 | E6 수직 L(2cpu) | CPU 벽 걷으니 갈라짐 — redis VUS2000 3,007rps 스케일 vs pessimistic 정체(~1,300)+커넥션타임아웃 614건(DB hot-row+풀 천장). 결론=스케일 의존(소형→pessimistic, 2cpu+/scale-out→redis) | [E6 크로스오버](experiments/E6-crossover-concurrency.md)
- 2026-08-27 | redis 제품화 | E6 결론(스케일=redis) 반영 — 잔여수량 count 기반 정확화(GET, redis remaining stale 대응), 크래시 갭 조정(redis 발급자 set↔DB 대조 전진복구, 멱등), README 데이터 기반 포트폴리오 얼굴로 갱신 | [redis-atomic-stock](concepts/redis-atomic-stock.md)
- 2026-08-27 | E7 멱등성 구현 | IdempotencyService(Redis SET NX 클레임+응답재생), /issue 전용(issue-and-notify 제외, D-006). Jackson2/3 공존 함정 발견·해결(P-002). 20 tests passed | [journal](journal/2026-08-27-E7-idempotency.md)
- 2026-08-28 | 조정 초과발급 버그 수정 | 30초 주기 조정이 살아있는 in-flight 발급과 경합→보상 INCR→유령 재고→초과(크래시 무관, 리뷰 지적). 기동 1회(ApplicationReadyEvent)로 변경 — 기동 직후 in-flight 없어 경합 원천 차단. @Scheduled·CouponSchedulingConfig 제거. 멀티노드는 유예-주기(E12) | [journal](journal/2026-08-27-e6-flash-sale.md)
- 2026-09-01 | E7 커버리지 보강 + 부하 실측 | 엔지니어링 리뷰 후속 — D4 회귀·action예외·InProgress→409 테스트 3건, retry-on 2000→201 중복0·storm 4000→발급1·409 in_progress 199. 25 tests. PR #5(d62f052) | [journal](journal/2026-08-27-E7-idempotency.md)
- 2026-09-01 | E8 Redis 저항성 | command timeout(1s) + 프로그래매틱 Resilience4j 서킷브레이커. DEAD(pause/stop)=timeout이 1s로 bound·pileup0·자동복구 / SLOW(+400ms)=브레이커 없이 2.9~5.3s 느린-성공 → OPEN시 fast-fail 503 p50 0.955ms. 28 tests. replica/Sentinel은 다음 슬라이스 | [E8](experiments/E8-redis-resilience.md) · [D-008](decisions/D-008-redis-resilience.md) · [journal](journal/2026-09-01-E8-redis-resilience.md)
- 2026-09-01 | E8 replica/Sentinel | redis+replica+sentinel×3(quorum 2), B override(docker-compose.sentinel.yml)+프로파일 게이트(application-sentinel.yml, 단일모드 공존, 앱 코드 0). advisor대로 Docker 주소 해석 격리검증→수동 failover→앱 following. primary stop 후 **앱이 ~4s만에 승격된 replica로 자동 재연결**. two-layer 완성(다운 동안=브레이커, 다운 길이=Sentinel) | [E8](experiments/E8-redis-resilience.md) replica/Sentinel 절
- 2026-09-01 | P-004 ghost 발급 재현·완화 | 리뷰 지적 검증 — Redis 응답 지연(>timeout) 시 서버는 Lua 실행했는데 앱이 포기 → 재고 깎이고 명단 있는데 DB 없는 ghost, 재시도 409 잠김. toxiproxy downstream latency로 확정(pause는 재현 안 됨=명령 취소, 헤더는 claim 게이트로 부분방어). 완화: timeout 1s→3s + coupon_issue_ambiguous 카운터+로그(발동 검증). 근본수정=조정 롤백 별도 PR | [P-004](problems/P-004-redis-timeout-ghost-issue.md)
